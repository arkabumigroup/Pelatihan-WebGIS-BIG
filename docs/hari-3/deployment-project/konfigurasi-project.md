...
| # | Berkas | Yang dilakukan | Wajib? |
|---|---|---|---|
| 1 | `01-schema.sql` | Membuat tiga tabel: `users`, `katalog_data_2d`, dan `katalog_data_3d` | Ya |
| 2 | `02-seed-super-admin.sql` | Membuat satu akun super admin untuk login pertama | Ya |
| 3 | `03-periksa.sql` | Memeriksa hasilnya, hanya membaca | Boleh dilewati |

Isi ketiga berkas itu ditampilkan di bawah, supaya Anda dapat menyalinnya langsung dari halaman ini. Berkas aslinya juga ada di folder `sql/` pada repositori Anda, dan isinya sama.

Jangan menyalin sebagian, karena beberapa berkas memakai `BEGIN` dan `COMMIT` yang harus berpasangan.

### 01-schema.sql

Menyalin berkas ini sekaligus membuat ketiga tabel. Aman dijalankan lebih dari sekali, karena memakai `CREATE TABLE IF NOT EXISTS`.
...
-- Berkas lib/auth dan konfigurasi NextAuth.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- katalog_data_2d
-- Kolom mengikuti ".../File latihan/katalog_data_2d.csv".
-- ---------------------------------------------------------------------
...
Isinya adalah template. Dua nilai di dalamnya masih berupa penanda, dan harus Anda ganti lebih dahulu:
...
Cara mengisinya ada pada bagian **Buat akun super admin** di bawah.
